        ...
        if (res.ok) {
          ...
          if (posts && posts.length > 0) {
            const businessPosts = posts.filter(
              (p: BlogPost) => p.category === 'business' || p.category === 'notion'
            );
            setArticles(businessPosts.map(transformPost));
          } else {
            const businessSeeds = seedPosts.filter(
              (p) => p.category === 'business' || p.category === 'notion'
            );
            setArticles(businessSeeds.map((post) => ({
              slug: post.slug,
              title: post.title,
              excerpt: post.excerpt || '',
              coverImage: post.cover_image || '',
